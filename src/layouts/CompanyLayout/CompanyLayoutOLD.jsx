  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (userMenuRef.current && !userMenuRef.current.contains(event.target)) {
        setIsUserMenuOpen(false) // Close the user menu when clicking outside
      }
    }
            ...
            <div style={{ position: 'relative' }}>
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center' }}>
                  <img src="/placeholder.svg?height=40&width=40" alt="User profile" style={styles.profilePhoto} />
                  Username
                </div>
                ...
                <div style={{ marginTop: '0.5rem' }}>
                  {/* {userMenuItems.map(({item, route}) => (
                        <NavLink
                          key={route}
                          to={`/company/${route}`}> */}
                  {[t('profile'), t('settings'), t('logout')].map((item) => (
                    <a
                      key={item}
                      href="#"
                      style={{
