...
import { handleFileChange, handleSubmit, handleDownload, submitApplicationByType } from '../../utils/secretaryUtils';
import LogoPlaceholder from '../../assets/logo_placeholder.png';
import CustomAlertDialog from '../ui/custom_alert';

export function PendingApplicationCard({ application, onSubmitSuccess }) {
  ...
  const companyName = announcement?.Company.name || application?.companyName;
  const companyProfile = announcement?.Company.CompanyProfile || {};
  const fileType = application?.isManual ? "ManualApplicationForm" : "ApplicationForm";
      ...
      >
        <div className="lg:col-span-2 flex items-center gap-4">
          <div className="w-16 h-16 rounded-full overflow-hidden bg-gray-200 flex items-center justify-center">
            <img
              src={companyProfile.companyLogo ? `http://localhost:3005/${companyProfile.companyLogo}` : LogoPlaceholder}
        ...
        <div className="flex-1">
          <p className="font-bold text-gray-700">
            {t('student')} <span className="font-normal">{student.username}</span>
          </p>
          <p className="font-bold text-gray-700 mt-2">
            {t('companyTitle')} <span className="font-normal">{companyName}</span>
          </p>
          <p className="font-bold text-gray-700 mt-2">
            {t('announcement')} <span className="font-normal">{announcement.announcementName}</span>
          </p>
          <p className="font-bold text-gray-700 mt-2">
            {t('startDate')}{' '}
        ...
        <div className="flex flex-col gap-3 min-w-[130px]">
          <div className="flex flex-col md:flex-row lg:flex-col xl:flex-row gap-3">
            <label
              ...
              onClick={async () => {
                try {
                  await handleDownload(application.id, fileType);
                } catch (error) {
          ...
          {fileSelected && (
            <div className="flex justify-end">
              <button
                ...
                onClick={async () => {
                  try {
                    await submitApplicationByType(application, selectedFile);
                    setSelectedFile(null);
                    setSelectedFileName('');
                    setFileSelected(false);
