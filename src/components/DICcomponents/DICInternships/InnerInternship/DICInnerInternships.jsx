...
const DICInnerInternships = () => {
  ...
  const getLatestFeedback = (feedbackArray) => {
    if (!Array.isArray(feedbackArray) || feedbackArray.length === 0) return null
  ...
  const handleStudentFeedbackSubmit = async () => {
    if ((!isManualApplication && !rejectionReason) || !studentFeedback.trim()) {
    ...
    try {
      const newFeedback = {
        ...
        feedbackContextStudent: isManualApplication ? "Survey" : rejectionReason,
        feedbackToCompany: null,
        feedbackContextCompany: null,
      }
      await evaluateInternship(id, newFeedback)
      alert(`${t("success")}: ${t("studentFeedbackSent")}`)
      setStudentFeedbackVisible(false)
      setStudentFeedback("")
      setRejectionReason("")
    } catch (error) {
      alert(`${t("error")}: ${t("failedToSendFeedback")}`)
    } finally {
  ...
  const handleCompanyFeedbackSubmit = async () => {
    if ((isManualApplication && !companyRejectionReason) || !companyFeedback.trim()) {
      setReasonAlertOpen(true)
      return
    }
    ...
    try {
       const newFeedback = {
        ...
        feedbackContextCompany: isManualApplication ? companyRejectionReason : "CompanyForm",
      }
      await evaluateInternship(id, newFeedback)
      alert(`${t("success")}: ${t("companyFeedbackSent")}`)
    } catch (error) {
      alert(`${t("error")}: ${t("failedToSendFeedback")}`)
    } finally {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      alert(`${t("error")}: ${t("failedToApprove")}`)
    } finally {
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      alert(`${t("error")}: ${t("failedToReject")}`)
    } finally {
  ...
  const handleDocumentDownload = async (documentType) => {
    try {
      ...
      if (!documentData.isAvailable || !documentData.id) {
        alert(`${t("error")}: ${t("documentNotAvailable")}`)
        return
      }
      ...
      alert(`${t("success")}: ${t("documentDownloaded")}`)
    } catch (error) {
      console.error("Download error:", error)
      alert(`${t("error")}: ${t("downloadFailed")}`)
    }
  }
          ...
          <div className="grid lg:grid-cols-2 gap-8 p-6">
            ...
            <Card className="border-2 border-blue-100 shadow-lg">
              ...
              <CardContent className="p-6">
                  ...
                  {/* Always display Latest Student Feedback if available */}
                  {latestStudentFeedback && (
                    <div className="p-4 bg-blue-50 rounded-lg border border-blue-200 mb-6"> {/* Added mb-6 for spacing */}
                  ...
                  {studentFeedbackVisible && (
                    <div className="mt-6 space-y-3">
                      {/* Rejection Reason (only for non-manual application) */}
                      {!isManualApplication && (
            ...
            <Card className="border-2 border-green-100 shadow-lg">
              ...
              <CardContent className="p-6">
                ...
                {/* Always display Latest Company Feedback if available */}
                {latestCompanyFeedback && (
                  <div className="p-4 bg-green-50 rounded-lg border border-green-200 mb-6"> {/* Added mb-6 for spacing */}
                    <div className="flex items-center justify-between mb-2">
                ...
                {/* New Feedback Input (only if not hidden by companyStatus === 4 and companyFeedbackVisible is true) */}
                {!hideCompanyControls && companyFeedbackVisible && (
                  <div className="mt-6 space-y-3">
                    {isManualApplication && (
                      <div className="p-4 bg-red-50 rounded-lg border border-red-200">
                        <label className="text-sm font-medium text-red-800 block mb-2">
                        ...
                        >
                          <option value="">{t("selectReason")}</option>
                          <option value="Report">{t("reportIssue")}</option>
                          <option value="CompanyForm">{t("companyFormIssue")}</option>
                          <option value="Both">{t("bothIssues")}</option>
                        </select>
                      </div>
                    )}
